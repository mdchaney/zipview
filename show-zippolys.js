...
function draw_zip_polys(svg, zip_polys, bounding_box, zip_range) {
	for (var arr of zip_polys) {
		const zip = arr[0];
		const points = arr[1];
		const color = zip_to_color(parseInt(zip), zip_range);
		var poly = document.createElementNS("http://www.w3.org/2000/svg", "polygon");
		poly.setAttribute("points", points.map(function(pair) { var xy = latLonToOffsets(pair[0], pair[1], 100, 100); return [xy.x,xy.y].join(','); }).join(' '));
		poly.classList.add('zip');
		poly.classList.add(zip);
